...
\section{General}
Agriculture represents the cornerstone of the global economy, supplying vital sustenance, raw materials, and livelihoods to billions worldwide. In developing nations like India, the agricultural sector employs over half the national workforce and contributes significantly to the country's Gross Domestic Product (GDP). However, agricultural yield remains constantly threatened by biological pathogens, environmental stressors, and invasive pests. Pathological infections—predominantly fungal, bacterial, and viral diseases—inflict catastrophic crop losses, resulting in an annual reduction of approximately 15\% to 20\% in India's total agricultural output.

Early identification of crop diseases is imperative to mitigate these losses. Traditionally, farmers diagnose plant pathologies through visual inspection of leaf symptoms. However, visual diagnostics are highly subjective, error-prone, and labor-intensive. Moreover, accessing professional plant pathologists remains difficult in remote, rural farming communities. Consequently, inaccurate diagnoses often prompt farmers to apply incorrect chemical pesticides, which not only escalates production costs but also causes soil degradation and environmental toxicity.

Automating disease detection using computer vision and deep learning techniques offers a scalable, objective solution. Convolutional Neural Networks (CNNs) have shown state-of-the-art performance in image classification. Nonetheless, most academic models are developed using pristine, laboratory-captured datasets, leading to a performance collapse when deployed in natural, unstructured field environments. To address this domain gap, we present a unified system featuring: (i) a custom, lightweight CNN backbone called ``AgriNet''; (ii) a mixed-domain training pipeline for domain adaptation; (iii) Explainable AI (XAI) overlays via Grad-CAM; and (iv) a Retrieval-Augmented Generation (RAG) assistant that provides context-grounded, verified agricultural advice.
...
\section{General}
A deep learning classifier's capability is fundamentally bound to the quality and diversity of its training data. Exposing neural networks to varied environmental conditions is critical for achieving robust generalization in agricultural computer vision. This chapter details our primary and secondary datasets, the image preprocessing pipeline, data augmentation techniques, and the class mapping logic implemented to fuse laboratory and field imagery.

\section{Primary Dataset: PlantVillage}
We utilized the PlantVillage dataset \cite{plantvillage} as our primary training source. This corpus contains 54,305 high-resolution, color images of healthy and diseased crop leaves across 14 plant species (e.g., Apple, Corn, Potato, Tomato, and Grape), distributed among 38 biological classes.

Importantly, all PlantVillage photos were captured under controlled laboratory conditions, featuring single leaves detached and laid flat against uniform grey backgrounds under consistent studio lighting. While this simplifies the extraction of clear pathological features, the total lack of background variation limits the network's generalization capability when exposed to unstructured, outdoor conditions.
...
Standard 2D convolutions demand substantial computational resources and parameters, hindering deployment on edge devices and mobile phones. To alleviate this bottleneck, we designed AgriNet as a lightweight, resource-efficient alternative. The foundational building blocks of AgriNet are Depthwise Separable Convolutions and Inverted Residual Blocks (AgriBlocks) \cite{mobilenetv2}.

\subsection{Depthwise Separable Convolutions}
Depthwise separable convolutions minimize the parameter footprint and multiplication operations by factorizing a standard convolution into two distinct stages: (i) a depthwise convolution, which applies a single spatial filter to each input channel independently, and (ii) a pointwise convolution ($1 \times 1$), which linearly projects the depthwise outputs into the target channel space.
...
Assuming a standard kernel size of $D_K = 3$, this decomposition reduces the computational complexity by a factor of 8 to 9 depending on the output channel dimension $N$, without significant degradation in classification performance.

\subsection{Inverted Residual Block (AgriBlock)}
The AgriBlock serves as the core feature extraction unit of AgriNet, structured as a three-stage block. First, an expansion pointwise convolution ($1 \times 1$) projects the input to a higher-dimensional space based on the expansion factor $t$. Second, a depthwise convolution ($3 \times 3$) extracts spatial features in this high-dimensional space. Third, a projection pointwise convolution ($1 \times 1$) projects the features back down to the target dimension. To prevent information loss and maintain linear representation, we omit the non-linear activation function after the final projection step, forming a linear bottleneck.

An identity shortcut connection is introduced to sum the block input and output. This skip connection is only applied when the stride equals $1$ and the input channels match the output channels. The final network architecture consists of an initial convolutional stem, four sequential AgriBlocks with increasing channel depths, a Global Average Pooling layer, a Dropout layer (configured to $0.3$), and a final fully connected classification layer, as outlined in Table~\ref{tab:agrinet_layout}.
...
We developed an automated pipeline to ingest official agricultural publications (e.g., Tomato \cite{nhb_tomato}, Potato \cite{nhb_potato}, Maize \cite{seedco_maize}, Apple \cite{vikaspedia_apple}) and Integrated Disease Management manuals \cite{idm_concept}. The system extracts raw text from these PDFs and processes them using LangChain's \texttt{RecursiveCharacterTextSplitter}. The chunk size is configured to $1,000$ characters with a $200$-character sliding overlap. This overlap maintains semantic continuity across boundary points.

\subsection{Vector Database and Retrieval Mechanics}
To perform fast similarity search, we embed the text chunks into dense $384$-dimensional vectors using a Sentence-Transformers model. These embeddings are stored locally in a Facebook AI Similarity Search (FAISS) vector index. When a user submits an inquiry, the query is mapped to the same embedding space, and the system runs a cosine-similarity search to retrieve the top $k = 3$ most relevant document chunks.

\subsection{Context-Grounded Generation (Groq Llama-3.1)}
The retrieved document chunks are formatted alongside the user query within a strictly constrained system prompt. This prompt is transmitted to the \texttt{llama-3.1-8b-instant} model via the Groq API. The model is explicitly instructed to synthesize answers solely from the retrieved context; if the context lacks relevant information, the model states that it cannot answer. This setup ensures that all responses are context-grounded and contain references to the source documentation, minimizing hallucinations.
...
\section{General}
This chapter delineates the architectural design and structural implementation details of the AgriVision application, including data flow, database models, and hardware specifications.

\section{Data Flow Diagrams (DFDs)}
Data Flow Diagrams (DFDs) track the progression and transformations of data throughout the system.

At the Context Level (Level 0), the system acts as a monolithic interface connecting the user, the database, and the vector index. Farmers upload leaf images and input text queries. The system outputs classification predictions, Grad-CAM overlays, RAG responses, and transaction logs.

The Level 1 DFD decomposes the system into three primary processes: (i) user authentication, which verifies credentials and issues JWTs; (ii) image prediction, which processes uploads through AgriNet, computes Grad-CAM heatmaps, and saves logs to PostgreSQL; and (iii) the RAG chat loop, which routes user queries through the FAISS vector database and LLM APIs. Figure~\ref{fig:dfd} illustrates these DFD levels.
...
The system utilizes a decoupled client-server architecture to ensure high performance, security, and scalability. The separation of concerns between the presentation layer, the application logic, and the machine learning/retrieval subsystems is illustrated in the System Architecture Diagram in Figure~\ref{fig:system_architecture}.
...
\begin{enumerate}
    \item \textbf{Presentation Layer (Frontend):} Built using Next.js 14 and React. It serves as a pure client and manages the presentation layer using custom vanilla CSS for glassmorphic visuals, with Framer Motion handling smooth layout transitions. It communicates with the backend via REST APIs for authentication, prediction requests, and conversational chat queries.
    \item \textbf{Application Logic Layer (Backend):} Powered by FastAPI on Python 3.11. This layer acts as the central orchestrator. It manages API routing, handles secure user authentication using JSON Web Tokens (JWT) and password hashing with Bcrypt, manages request/response payloads, and coordinates services between the machine learning models and the database.
    \item \textbf{Data and AI Engine Layer:} Consists of three core sub-components:
    \begin{itemize}
        \item \textbf{PostgreSQL Database:} Persistent storage for user records, prediction histories, and diagnostic statistics, integrated directly with the backend using the Python Prisma Client (\texttt{prisma-client-py}).
        \item \textbf{Deep Learning Classifier (AgriNet + Grad-CAM):} Executes the lightweight PyTorch model for disease classification and generates explainability heatmaps using Grad-CAM.
...
We systematically tracked the performance of the custom AgriNet model during both the initial base training phase and the subsequent mixed-domain fine-tuning stage.

\subsection{Phase 1 Base Model Training Results}
During the initial phase on the pristine PlantVillage corpus, the network exhibited rapid convergence, reaching a validation accuracy of $94.68\%$ by the second epoch. By the final epoch ($10$), the model attained its lowest validation loss of $0.0396$ and a peak validation accuracy of $98.75\%$. These results indicate that the Inverted Residual Blocks effectively extract core leaf features when background noise is minimized. The detailed epoch-by-epoch training progress is reported in Table~\ref{tab:phase1_metrics}.
...
Phase 2 introduced the challenging, noisy PlantDoc images into the training pipeline. Although natural backgrounds, occlusion, and variable illumination increased training difficulty, the model converged within $9$ epochs, achieving a mixed-domain validation accuracy of $96.14\%$. This validates the effectiveness of our domain adaptation approach in handling natural field environments. Table~\ref{tab:phase2_metrics} provides the fine-tuning training metrics.
...
We evaluated classification performance using standard statistical metrics: Precision, Recall, and F1-score. Precision measures the ratio of true positive predictions to all predicted positives; Recall measures the ratio of true positives to all actual positive instances; and the F1-score is computed as their harmonic mean. On the combined validation dataset, the model achieved a weighted average of $0.99$ across all three metrics. The corresponding 38-class confusion matrix is illustrated in Figure~\ref{fig:confusion_matrix}.
